       ...
       public (Vector2, Array<Dictionary>) GetTargetColliders(Vector2 targetPosition)
{
    ...
    // Проверка следующей клетки относительно середины
    Vector2 adjustedTargetPosition = targetPosition;
    adjustedTargetPosition.X += Globals.GRID_SIZE / 2; // = 8
    adjustedTargetPosition.Y += Globals.GRID_SIZE / 2; // = 8
    ...
    return (adjustedTargetPosition, spaceState.IntersectPoint(query));
		...
		public bool IsTargetOccupied(Vector2 targetPosition)
		{
			...
			{
				var collider = (Node)(GodotObject)result[i]["collider"];
				var colliderPos = ((Node2D)collider).GlobalPosition;
			...
			if (result.Count <= 2)
			{
				...
				{
					int nonPlayerColliders = 0;
					foreach (var col in result)
					{
						var c = (Node)(GodotObject)col["collider"];
						if (c.GetType().Name != "Player")
						{
							nonPlayerColliders++;
						}
					}

					if (nonPlayerColliders == 0)
					{
						return false;
					}
        ...
        public bool GetTileMapLayerCollision(TileMapLayer tileMapLayer, Vector2 adjustedTargetPosition)
        {
            ...
            Variant ledgeData = tileData.GetCustomData("LEDGE");
            if (ledgeData.VariantType != Variant.Type.Nil)
            {
                string ledgeDirection = ledgeData.AsString();
                
                if (!string.IsNullOrEmpty(ledgeDirection))
                {
        ...
        public void Walk(double delta)
        {
            if (IsWalking)
            {
                Character.Position = Character.Position.MoveToward(
                    TargetPosition, 
                    (float)delta * Globals.GRID_SIZE * 4
                );
        ...
        public void StopMoving()
        {
            ...
            {
                GameState.Instance.PlayerPosition = Character.Position;
            }
        }
        public void Turn()
        {
